...
class Save extends \Amasty\CustomerAttributes\Controller\Adminhtml\Attribute
{
    ...
    public function execute()
    {
        ...
        try {
            ...
            $model->save();
            $this->processImages($model->getId());
    ...
    private function validateData($data, \Magento\Customer\Model\Attribute $model)
    {
        ...
        $data['store_ids'] = '';
        $data['sort_order'] = (int) $data['sorting_order'] + CustomerFormManager::ORDER_OFFSET;//move attributes to the bottom
    ...
    private function processImages($modelId)
    {
        /** Deleting */
        ...
        if ($files) {
            ...
            foreach ($files as $optionId => $file) {
                ...
                if (isset($options[$optionNumber]['option_id'])) {
                    $saveOptionId = $options[$optionNumber]['option_id'];
                }
                $optionNumber++;
                if (UPLOAD_ERR_OK == $file['error']) {
                    $this->_imageHelper->uploadImage($optionId, $saveOptionId);
                }
            }
        }
    }
