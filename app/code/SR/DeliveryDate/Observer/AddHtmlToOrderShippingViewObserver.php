...
class AddHtmlToOrderShippingViewObserver implements ObserverInterface
{
    ...
    public function execute(EventObserver $observer)
    {
        if($observer->getElementName() == 'order_shipping_view') {
            ...
            if($order->getShippingDeliveryDate() != '0000-00-00 00:00:00') {
                $formattedDate = $localeDate->formatDateTime(
                    $order->getShippingDeliveryDate(),
            ...
            $deliveryDateBlock->setShippingDeliveryDate($formattedDate);
            $deliveryDateBlock->setShippingDeliveryComment($order->getShippingDeliveryComment());
